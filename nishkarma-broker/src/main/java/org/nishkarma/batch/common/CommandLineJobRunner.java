	...
	int start(String jobPath, String jobIdentifier, String[] parameters, Set<String> opts) {
		...
		try {
			...
			if (opts.contains("-stop")) {
				...
				logger.info("-1---ExitStatus.COMPLETED");
			...
			if (opts.contains("-abandon")) {
				...
				logger.info("-2---ExitStatus.COMPLETED");
				return exitCodeMapper.intValue(ExitStatus.COMPLETED.getExitCode());
			}
			...
			logger.info("-3---jobExecution="+jobExecution);
			logger.info("-3---jobExecution.getExitStatus()="+jobExecution.getExitStatus());
			logger.info("-3---jobExecution.getStatus()="+jobExecution.getStatus());
			
			logger.info("-3---jobExecution.getExitStatus().getExitCode()="+jobExecution.getExitStatus().getExitCode());
